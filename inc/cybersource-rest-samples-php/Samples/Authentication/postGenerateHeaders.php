...
class PostGeneratorHeader
{
    ...
    {
        $getConfig         = new CyberSourceApi\Authentication\Util\PropertiesUtil();

        $requestJsonPath   = __DIR__ . DIRECTORY_SEPARATOR . '../../Resources/authRequest.json';

        $merObj            = new CyberSourceApi\ExternalConfiguration();
        $merchantConfigObj = $merObj->merchantConfigObject();

        $payload           = new CyberSourceApi\Authentication\PayloadDigest\PayloadDigest();
        ...
        try {
            $auth         = new CyberSourceApi\Authentication\Core\Authentication();
