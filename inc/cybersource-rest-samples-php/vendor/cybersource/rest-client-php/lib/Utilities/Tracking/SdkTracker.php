...
namespace CyberSourceApi\Utilities\Tracking;
...
class SdkTracker
{
    ...
    public function insertDeveloperIdTracker($requestObj, $requestClass, $runEnvironment)
    {
        if (in_array($requestClass, self::$inclusionList))
        {
            ...
            {
                if (!empty($requestObj->getClientReferenceInformation()->getPartner()) && method_exists($requestObj->getClientReferenceInformation()->getPartner(),'getDeveloperId'))
                {//var_dump($requestObj->getClientReferenceInformation()->getPartner());exit;
