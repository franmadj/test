<?php
add_action('wp_ajax_location_search', 'location_search_callback');
add_action('wp_ajax_nopriv_location_search', 'location_search_callback');

function location_search_callback() {
    global $wpdb;
    $search_radius = $_POST['distance'];
    $string = $_POST['address'];
    if ($string == 'usa' || $string == 'USA' || $string == 'Usa') {
    ...
    $i = 1;
    if ($string == '') {
        get_template_part('template-parts/location', 'marker');
        ?>
    ...
    $location = new WP_Query($query_args_meta);
    if ($location->have_posts()) :
        while ($location->have_posts()) : $location->the_post();

            //get_template_part('template-parts/map','marker');
            if ($location->post_count == $i) {
                include(get_template_directory() . '/template-parts/radius-search.php');
            }
        ...
        $address = file_get_contents("https://maps.googleapis.com/maps/api/geocode/json?key=AIzaSyAR0JcTIISM1lfgw7BDmHe97lva61zGKKY&address=$new_string");
        $formated_adderess = json_decode($address);


        if ($formated_adderess && isset($formated_adderess->results[0]) && isset($formated_adderess->results[0]->geometry->location)) {


            function error_log____($value) {
                if ($_SERVER['REMOTE_ADDR'] == '77.228.116.64') {
                    error_log($value);
                }
            }

            error_log____('$formated_adderess' . print_r([$new_string, $formated_adderess], true));
            global $wpdb;
            $location_latlong = $formated_adderess->results[0]->geometry->location;
            $location_lat = $location_latlong->lat;
            $location_long = $location_latlong->lng;
            ...
            if ($post_ids) {
                ...
                foreach ($post_ids as $key => $value) {
                    ...
                    if ($map_location):
                        ...
                        ?>","distance":"<?php echo (int) $value->distance; ?> mi. away","link":"<?php echo get_the_permalink($key); ?>"}'>
                                  <?php if ($slide_images): ?>
