...
class SendEgiftManuallyTest {
    ...
    private static function init_hooks() {
        self::$initiated = true;
        if (!empty($_GET['send-egift-manually-test']))
    ...
    public static function getEmailPdf() {
        if (empty($_GET['download_egift_pdf_file']))
            return;
        // Initialize a file URL to the variable
        $url = $_GET['download_egift_pdf_file'];
        global $wpdb;


        //CHECK NORMAL CARDS
        $fileUrlNameCard = WP_CONTENT_URL . '/uploads/email-templates/texas-email-pdf-';
        if (strpos($url, $fileUrlNameCard) !== false) {
            $code = str_replace($fileUrlNameCard, '', $url);
            $code = str_replace('.html', '', $code);
            if (strlen($code) == 4) {
                $sql = "SELECT count(meta_id) c FROM ".$wpdb->prefix."woocommerce_order_itemmeta WHERE meta_key = 'paytronixRegCode' AND meta_value like '%$code%'";
                $res = $wpdb->get_col($sql);
                if (!empty($res[0]) && $res[0] > 1) {
                    //wp_mail('franworkspace@gmail.com', 'INVALID_PDF_REDIRECT', $_GET['download_egift_pdf_file']);
                    wp_redirect(INVALID_PDF_REDIRECT);
                    return;
                }
            }
        }
        ...
        //CHECK BONUNS CARDS
        $fileUrlNameBonus = WP_CONTENT_URL . '/uploads/email-bonus-templates/texas-email-pdf-';
        if (strpos($url, $fileUrlNameBonus) !== false) {
            $code = str_replace($fileUrlNameBonus, '', $url);
            $code = str_replace('.html', '', $code);
            if (strlen($code) == 4) {
                $sql = "SELECT count(meta_id) c FROM ".$wpdb->prefix."woocommerce_order_itemmeta WHERE meta_key = 'BonusRegCode' AND meta_value like '%$code%'";
                $res = $wpdb->get_col($sql);
                if (!empty($res[0]) && $res[0] > 1) {
                    //wp_mail('franworkspace@gmail.com', 'INVALID_PDF_REDIRECT', $_GET['download_egift_pdf_file']);
                    wp_redirect(INVALID_PDF_REDIRECT);
...
//        exit;
        ...
        $mpdf = new \Mpdf\Mpdf();
        $content = trim(self::tx_file_get_contents($url . '?a=56'));
//        var_dump([$content, $url . '?a=56']);
//        exit;
        ...
        if (file_put_contents($file_name, self::tx_file_get_contents($url))) {
    ...
    public static function tx_file_get_contents($path) {

        $remote_svg_file = wp_remote_get($path);
    ...
    public static function notifyRecipient($cardNumber, $regCode, $slug, $amount) {
        ...
        $showPdfLinks = false;
        $page = include(get_template_directory() . '/create-bonuscard/texas-email-template-test.php');
        ...
        $showPdfLinks = true;
        $body = include(get_template_directory() . '/create-bonuscard/texas-email-template-test.php');
...
add_action('init', array('SendEgiftManuallyTest', 'init'));
