...
while(the_flexible_field("block_body")):
	...
	if(get_row_layout() == "promo"): ?>
      ...
      <div class="o-col">
      	...
      	<section class="home-group-dining <?php if(get_sub_field('alignment') == 'right' ){ ?> u-right-promo<?php }?>" style="background-image: url(<?php echo $promo_desktop_image;?>)" data-ref="responsiveVisualElement" data-mobile="<?php echo $promo_mobile_image;?>" data-desktop="<?php echo $promo_desktop_image;?>">
            <div class="home-group-dining__inner">
                ...
                if(have_rows('button')):
                	while(have_rows('button')):the_row();?>
										<a href="<?php the_sub_field('button_url');?>" class="button <?php if(get_sub_field('style_type') == 'accent'){?> button--accent-two <?php }?> home-group-dining__cta" <?php if(get_sub_field('button_target') == 'blank'){?> target="_blank" <?php } ?> data-ref="openModal" data-type="hosting" data-ignore><?php the_sub_field('button_text');?></a>
								<br>
								<?php endwhile;?>
							<?php endif;?>
            </div>
...
<script type="text/javascript">
    ...
    var onloadCallback = function() {
        widgetId1 = grecaptcha.render(document.getElementById('recap1'), {
          'sitekey' : '6LcnlqEUAAAAAI2gOBufXKF1IBQ1Bcqh8jOzjNte'
        });
    ...
    $( '.hosting_submit' ).click(function(){
        response1 = grecaptcha.getResponse(0);
      
        if (response1.length === 0) {
            $( '.recap1-error').text( "reCAPTCHA is mandatory" );
            return false;
        }else{
            $( '.recap1-error').text('');
        }
    ...
    $( '.accounts_submit' ).click(function(){
        ...
        if (response3.length === 0) {
            $( '.recap3-error').text( "reCAPTCHA is mandatory" );
            return false;
        }else{
            $( '.recap3-error').text('');
        }
